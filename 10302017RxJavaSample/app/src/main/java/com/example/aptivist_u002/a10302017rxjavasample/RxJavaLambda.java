...
public class RxJavaLambda {
    public static void main(String[] args) {
        ...
        observable1.concatWith(observable2).filter(integer -> integer%2 ==0).subscribe(observer);

        observable3.map(integer -> String.valueOf(integer)).subscribe(stringObserver);

        observable3.map(integer -> integer*integer).subscribe(observer);

        observable1.zipWith(observable2, (integer, integer2) -> integer + " - " + integer2).subscribe(stringObserver);


        observable3.filter(integer -> integer % 2 == 1).subscribeOn(Schedulers.io())//new thread
